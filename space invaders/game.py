...
FIREGRENADE = pygame.mixer.Sound("sounds/fire grenade.mp3")
EXPLODEGRENADE = pygame.mixer.Sound("sounds/grenade explode.mp3")
...
def randnum(s = 1, e = 15, float = False):
    if not float: return random.randint(s, e);
    if float: return random.uniform(s, e);
    ...
    def kaplooey(this, deathDist = 100):
        ...
        for i in enemies.copy():
            if dist(this, i) < deathDist:
                i.deathAnim();
        for i in projectiles.copy():
            if not i == this:
                if dist(this, i) < deathDist:
                    i.delete = True;
        if not this.kaboomed: pygame.mixer.Sound.play(EXPLODEGRENADE);
        this.kaboomed = True;
        this.kaboomW = deathDist;
        this.xv = 0;
        this.yv = 0;
...
def createBlock(x = sW / 2, y = sH / 2, hp = 5.0, color = GREEN, img = TESTIMG, w = 16, h = 16):
    global blocks;
    n = obj(x,y,hp,color,img,w,h);
    blocks.append(n);
    
def createBlocks(x = sW/2, y = sH/2, hp = 5.0, img = TESTIMG, color = GREEN, amount = [5, 2], w = 16, h = 16):

     
    i = 0; l = amount[1]; # get num of columns
    i2 = 0; l2 = amount[0]; # get length of rows
    while i < l:

        while i2 < l2: # make the row
            tempX = int(x + i2*w);
            tempY = int(y + i*h);
            createBlock(x = tempX, y = tempY, hp = hp, color = color, img = img, w = w, h = h);
            i2 += 1;
        i2 = 0;
        i += 1;
...
createBlocks(x = sW/4 - 32, y = 372);
createBlocks(x = sW/4*2 - 32, y = 372);
createBlocks(x = sW/4*3 - 32, y = 372);
...
def createEnemyRow(x = 15, y = 45, count = 8, img = BLANK, color = GREEN, anim = False, column = [0,0,0]):
    ...
    for i in j: #idk how to use for loops in this way any other way
        while k < count:
            if randnum(0,1) == 1: tempImg = ALIEN1;
            else: tempImg = ALIEN2;
            createEnemy(xVal, yVal, img = tempImg, color = color, xv = xv, type = "norm", anim = anim);
            xVal += 64;
            k += 1;
...
def createProj(x = sW / 2, y = sH / 2, hp = 1.0, color = GREEN, img = BLANK, w = 3, h = 16, dmgT = "plr", yv = -5, xv = 0, pierce = False, powerUp = False):
    ...
    n.powerUp = powerUp;
    if n.dmgT == "grenade": n.fuse = 100; n.boomTime = 10; n.kaboomed = False;
...
plr = obj(sW / 2 - 32, sH - 32, 1.0, GREEN, PLAYERSHIP, 32, 25, deathImg = PLAYERDIE);
plr.tempW = plr.w;
plr.tempH = plr.h;
plr.shootDel = 0;
plr.shootCount = 1;
...
def plrInput():
    ...
    for i in blocks.copy():
        collBlock = collide(i.rect, plr.rect);
        if collBlock:
            plr.x = plr.px;
            plr.y = plr.py;
            plr.xv = 0;
    ...
    if keys[pygame.K_g] and plr.grenades < 3: plr.grenades = 3;
        # set player grenades to 3, removing other weapons \ deosn't need to happen
    ...
    if plr.y < 0: # push player outta the top limit of ur movement
        ...
        plr.yv = 0.0;
        # fire attack
    if keys[pygame.K_q] and plr.grenades > 0 and plr.grenadeCD == 0:
            tempColor = ORANGE;
            
            tempVel = -15;
            
            createProj(x = plr.x + plr.w/2 - 3, y = plr.y - 16, img = TESTIMG, dmgT = "grenade", yv = tempVel + 1, color = tempColor, w = 5, h = 5);
            pygame.mixer.Sound.play(FIREGRENADE);
            plr.grenades -= 1;
            plr.grenadeCD = 65;
            c.shakeStr = 10;
            c.shakeTime = 5;
    ...
    if (keys[pygame.K_SPACE] or mouseD) and plr.shootDel == 0: #shoot a bullet if there's not a playr bullet alread on screen
        ...
        if plr.laser and plr.laserAmmo > 0: 
            ...
            def smgthing():
                ...
                createProj(plr.x + plr.w/2-3, plr.y-16, color = PURPLE, img = TESTIMG, dmgT = "plr", yv = tempVel, xv = randnum(.5, -.5, True));
                c.shakeStr = 10;
                c.shakeTime = 3;
            setTimeout(smgthing, 0.1);
            setTimeout(smgthing, 0.2);
            setTimeout(smgthing, 0.3);
...
def plrFrame():
    ...
    plr.laserX1 = plr.x + plr.w/2 + c.shakeX;
    plr.laserY1 = plr.y + 3 + c.shakeY;
    if plr.laser:
        plr.laserX2 = mx + c.shakeX;
        plr.laserY2 = my + c.shakeY;
    else: plr.laserX2 = plr.laserX1; plr.laserY2 = c.shakeY;
    if plr.laserShoot: plr.laserColor = RED; plr.laserW = 5;
    else:
        plr.laserW = 1;
        if plr.laser: plr.laserColor = BLUE;
        elif plr.shotgun: plr.laserColor = RED;
        elif plr.smg: plr.laserColor =  PURPLE;
        ...
        for i in projectiles.copy():
            if collide(i.rect, plr.rect): # run things when projectile
                ...
                if i.powerUp:
                    ...
                    if i.dmgT == "life": plr.lives += 1;
                    if i.dmgT == "grenade": plr.grenades = 3;
                    i.delete = True;
...
def enemyFrame(this):
    ...
    if not this.dead and this.type == "norm":
        if this.y > sH or this.y < 0: this.delete = True;
...
def projFrame(this):
    ...
    if this.dmgT == "grenade" and not this.powerUp:
        ...
        if this.kaboomed: 
            if this.boomTime > 0:
                this.boomTime -= 1;
                if this.boomTime <= 9 and this.boomTime > 5: pygame.draw.circle(screen, WHITE, (this.x+this.w/2, this.y+this.h/2), this.kaboomW);
                if this.boomTime <= 5 and this.boomTime > 2: pygame.draw.circle(screen, ORANGE, (this.x+this.w/2, this.y+this.h/2), this.kaboomW);
                if this.boomTime <= 2: pygame.draw.circle(screen, YELLOW, (this.x+this.w/2, this.y+this.h/2), this.kaboomW);
            if this.boomTime == 0: this.delete = True;
        ...
        if not i == this and not this.powerUp and not i.powerUp:
            ...
            if not i.dmgT == this.dmgT:
                ...
                if collide(i.rect, this.rect):
                    ...
                    if not this.pierce: this.delete = True;
                    if not i.pierce: i.delete = True;
    for i in blocks.copy():
            if collide(this.rect, i.rect):
                this.delete = True;
...
def rmvLvlText():
    ...
    for i in text.copy():
        if i.special == "lvl": i.delete = True;
...
def screenThings():
    ...
    if gameState == "game":
        ...
        if not plr.dead: pygame.draw.line(screen, plr.laserColor, (plr.laserX1, plr.laserY1), (plr.laserX2, plr.laserY2), plr.laserW);
        spot = 0;
        for i in str(score):
...
def game():
    ...
    for i in enemies:
        if i.delete: enemies.remove(i);
    for i in projectiles:
        if i.delete: projectiles.remove(i);
    for i in blocks:
        if i.delete: blocks.remove(i);
